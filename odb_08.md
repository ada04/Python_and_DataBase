# 8. Использование переменных подстановки (bind)  #!

Операторы SQL и PL/SQL, передающие данные в базу данных Oracle и обратно, должны использовать плейсхолдеры в операторах SQL и PL/SQL, которые отмечают, где данные предоставляются или возвращаются. Плейсхолдер для переменной подстановки — это идентификатор или число с префиксом в виде двоеточия. Например, `:dept_id` и `:dept_name` — это два плейсхолдера для переменной подстановки в операторе SQL:
...
В процессе выполнения предоставленные значения переменных подстановки заменяются базой данных на `280` и `"Facility"`. Это называется подстановка или связывание (binding).

Использование переменных подстановки важно для масштабируемости и безопасности. Они помогают избежать проблем безопасности, связанных с SQL-инъекциями, поскольку данные при разборе никогда не рассматриваются как часть исполняемого оператора.

Переменные подстановки снижают затраты на анализ и выполнение, когда операторы выполняются более одного раза с разными значениями данных. Если переменные подстановки не используются, Oracle приходится повторно анализировать и кэшировать несколько операторов. При использовании переменных подстановки Oracle Database может повторно использовать план выполнения и контекст оператора.
...
Переменные подстановки можно использовать для подстановки данных, но нельзя использовать для подстановки текста оператора. Например, нельзя использовать плейсхолдеры переменных подстановки там, где требуется имя столбца или таблицы. Плейсхолдеры переменных подстановки также нельзя использовать в операторах DDL, таких как CREATE TABLE или ALTER.

## 8.1. Обязательство по имени или должности

Привязка может осуществляться «по имени» или «по позиции».

### 8.1.1. Привязка по имени

Привязка по имени выполняется, когда переменные привязки в операторе Python используют имена плейсхолдеров в операторе SQL или PL/SQL. Например:
...
Преимущества именованной привязки заключаются в том, что порядок значений привязки в параметре `execute()` не важен, имена могут быть осмысленными, а имена заполнителей могут повторяться, при этом значение в приложении по-прежнему предоставляется только один раз.
...
Позиционное связывание происходит, когда в вызов `execute()` передаётся список или кортеж значений подстановки. Например:
...
В следующем примере также используется привязка по позиции, несмотря на то, что плейсхолдеры имеют буквенные имена. Сам процесс привязки использует позиции входных данных в списке для подстановки данных с местоположением плейсхолдеров:
...
Если в операторе SQL или PL/SQL используется только один заполнитель привязки, данные могут быть списком `[280]` или кортежем из одного элемента `(280,)`.

## 8.2. Дублирующиеся заполнители переменных привязки

Связывание по имени рекомендуется в тех случаях, когда имена заполнителей переменных подстановки повторяются в операторах.

В Thin режиме python-oracledb при связывании по позиции для операторов SQL порядок значений привязки должен точно соответствовать порядку каждого заполнителя переменной привязки, а дублирующиеся имена должны иметь повторяющиеся значения:
...
В некоторых случаях Thick режим python-oracledb может допускать недублирующиеся значения для SQL-выражений, но такое использование не является последовательным и не рекомендуется. В Thin режиме python-oracledb это приведёт к ошибке.
...
## 8.3 Направление привязки (IN/OUT)

Вызывающий объект может предоставить данные в базу данных (IN), база данных может вернуть данные вызывающему объекту (OUT), или вызывающий объект может предоставить исходные данные в базу данных, а база данных может вернуть изменённые данные вызывающему объекту (IN/OUT). Это называется направлением подстановки.

Все представленные выше примеры предоставляют данные в базу данных и, следовательно, классифицируются как переменные подстановки IN. Чтобы база данных вернула данные вызывающей стороне, необходимо создать переменную. Это делается путем вызова метода `Cursor.var()`, который, помимо прочего, определяет тип данных, которые будут найдены в этой переменной подстановки, и её максимальный размер.

Вот пример использования OUT-связываний. Он вычисляет сумму целых чисел 8 и 7 и сохраняет результат в переменной OUT-подстановки типа integer:
...
Чтобы вставить значение NULL в символьный столбец, можно подставить одиночное значение Python -  `None`. Например, с таблицей:
...
Обратите внимание, что этот тип `oracledb.DB_TYPE_UROWID` поддерживается только в Thin режиме python-oracledb. В Thick режиме python-oracledb тип базы данных UROWID может быть связан с типом `oracledb.DB_TYPE_ROWID`. См. [раздел Запрос метаданных в тонком и толстом режимах](odb_30.md#30_4).

## 8.7. Возврат значений DML операций в переменные подстановки

При использовании предложения RETURNING с оператором DML, таким как UPDATE, INSERT или DELETE, значения возвращаются в приложение посредством переменных подстановки OUT. Рассмотрим следующий пример:
...
Одно и то же имя плейсхолдера переменной подстановки не может использоваться одновременно до и после предложения RETURNING. Например, если переменная подстановки `:dept_name` используется одновременно до и после предложения RETURNING:
...
В приведенном выше примере переменная привязки не будет обновлена ​​ожидаемым образом, но при использовании режима Thick python-oracledb ошибка не возникнет. В режиме Thin python-oracledb этот пример возвращает следующую ошибку:
...
Объекты CLOBs, NCLOBs, BLOBs и BFILEs базы данных могут быть связаны с типами `oracledb.DB_TYPE_CLOB`, `oracledb.DB_TYPE_NCLOB` и `oracledb.DB_TYPE_BLOB` соответственно. Извлеченные из базы данных или созданные с помощью `Connection.createlob()` объекты LOB, также могут быть связаны с типом `oracledb.DB_TYPE_BFILE`.
...
LOB-объекты можно использовать как переменные подстановки IN, OUT или IN/OUT.

Примеры см. в [разделе Использование данных CLOB, BLOB, NCLOB и BFILE](odb_12.md).

## 8.9. Переменные подстановки REF CURSOR
...
В учебной [схеме Oracle - HR](https://github.com/oracle/db-sample-schemas) есть два сотрудника с фамилией «Смит», поэтому результат следующий:
...
Чтобы вернуть REF CURSOR из функции PL/SQL, используйте тип `oracledb.DB_TYPE_CURSOR` для возвращаемых данных `Cursor.callfunc()`:
...
Информацию о настройке REF CURSORS см. в [разделе Настройка python-oracledb](odb_11.md).

См. также [раздел Неявные результаты](odb_07.md#7_8), который предоставляет альтернативный способ возврата результатов запроса из процедур PL/SQL.
...
Код Python для обработки коллекции OUT будет выглядеть следующим образом. Обратите внимание, что вызов `Cursor.arrayvar()` создаёт место для массива строк. Каждая строка может содержать до 100 байтов, и допускается только 10 строк. Если блок PL/SQL превысит максимально допустимое количество строк, возникнет ошибка `ORA-06513: PL/SQL: index for PL/SQL table out of range for host language array`.
...
Все коллекции, показанные в предыдущих примерах, использовали элементы непрерывного массива. Если требуется ассоциативный массив с разреженными элементами, следует использовать другой подход. Рассмотрим следующий код PL/SQL:
...
Обратите внимание, что индексы элементов коллекции разделены большими значениями. Применённый выше метод не сработает вызвав исключени: `ORA-06513: PL/SQL: index for PL/SQL table out of range for host language array`. Код, необходимый для обработки этой коллекции, выглядит следующим образом:
...
Обратите внимание, что использование метода `Object.aslist()` возвращает значения элементов коллекции в порядке индексов в виде простого списка Python. При таком подходе сами индексы теряются. Ассоциативный массив можно преобразовать в словарь Python с помощью метода `Object.asdict()`. Если бы это значение было выведено в предыдущем примере, вывод был бы следующим:
...
## 8.11 Связывание типа "запись" PL/SQL
...
## 8.12 Связывание типов данных Oracle Spatial
...
Пример выборки SDO_GEOMETRY можно найти в [Oracle Database Objects and Collections](odb_06.md#6_1_8).

## 8.13 Уменьшение количества версий SQL

При повторных вызовах `Cursor.execute()` или `Cursor.executemany()` связывании строковых данных разной длины с использованием `Cursor.setinputsizes()` может помочь сократить «количество версий» SQL-выражения в Oracle Database. Количество версий — это количество дочерних курсоров, используемых для одного и того же текста оператора. В базе данных будет родительский курсор, представляющий текст оператора, и несколько дочерних курсоров для различных вариантов выполнения оператора, например, при использовании переменных подстановки разных типов или длины.
...
Для уменьшения количества дочерних курсоров можно использовать `setinputsizes()`:
...
Однако если бы `setinputsizes()` не использовался, вы бы увидели две строки, а REASON включал бы текст «Несоответствие привязки», например:
...
Обработчик типа ввода включается путем установки атрибута `Cursor.inputtypehandler` или `Connection.inputtypehandler`.
...
Если запрос выполняется несколько раз с разным количеством значений, в SQL-оператор следует включить плейсхолдер для подстановки переменных для каждого из максимально возможного количества значений. Если оператор выполняется с меньшим количеством значений данных, следует выполнить связывание `None` для отсутствующих значений. Например, если запрос используется для пяти значений, но в конкретном выполнении используются только два, код может быть следующим:
...
Повторное использование одного и того же SQL-оператора, подобное этому, для переменного количества значений, вместо создания уникального оператора для каждого набора значений, позволяет максимально эффективно повторно использовать ресурсы Oracle Database. Кроме того, если оператор с большим количеством плейсхолдеров переменных подстановки выполняется много раз с разной длиной строк при каждом выполнении, рассмотрите возможность использования `Cursor.setinputsizes()`, чтобы уменьшить «количество версий» SQL-оператора Oracle Database для этого оператора. Например, если столбцы имеют тип VARCHAR2(25), добавьте перед вызовом `Cursor.execute()` следующее:

```
cursor.setinputsizes(25,25,25,25,25)
```

Если в операторе требуются другие переменные подстановки, соответствующим образом скорректируйте номера заполнителей переменных подстановки:
...
Одним из методов для больших списков IN является использование коллекции `TABLE()` Oracle Database. Например, если был создан следующий тип:
