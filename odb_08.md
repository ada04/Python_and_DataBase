...
Позиционное связывание происходит, когда в вызов передаётся список или кортеж значений подстановки execute(). Например:
...
Одно и то же имя плейсхолдера переменной подстановки не может использоваться одновременно до и после предложения RETURNING. Например, если :dept_nameпеременная подстановки используется одновременно до и после предложения RETURNING:
