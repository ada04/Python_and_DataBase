...
При повторных вызовах Cursor.execute()или Cursor.executemany() связывании строковых данных разной длины использование Cursor.setinputsizes() может помочь сократить « количество версий » SQL-выражения в Oracle Database для оператора. Количество версий — это количество дочерних курсоров, используемых для одного и того же текста оператора. В базе данных будет родительский курсор, представляющий текст оператора, и несколько дочерних курсоров для различных вариантов выполнения оператора, например, при использовании переменных подстановки разных типов или длины.
