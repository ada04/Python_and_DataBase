...
Python-oracledb обычно устанавливается из репозитория пакетов Python PyPI с помощью pip или uv .

Установите Python 3 , если он еще не доступен.

Используйте любую версию от Python 3.9 до 3.14.

Предыдущие версии python-oracledb поддерживали старые версии Python.

Установите python-oracledb, например:
...
with oracledb.connect(user=un, password=pw, dsn=cs) as connection:
    with connection.cursor() as cursor:
        ...
        for r in cursor.execute(sql):
            print(r)
Отредактируйте test.pyи задайте переменные unи csдля своего имени пользователя базы данных и строки подключения к базе данных соответственно.
...
В режиме python-oracledb Thick стандартное сетевое взаимодействие клиент-сервер Oracle Database обеспечивает взаимодействие между различными версиями клиентских библиотек Oracle и Oracle Database. Информация о текущих или ранее сертифицированных конфигурациях представлена ​​в документе Oracle Support с идентификатором 207303.1 . Вкратце:
...
Любая попытка использовать функции Oracle Database, не поддерживаемые определённым режимом или комбинацией клиентской библиотеки и базы данных, приведёт к ошибкам выполнения. Атрибут python-oracledb Connection.thinпозволяет определить режим соединения. В режиме Thick эта функция oracledb.clientversion()позволяет определить используемую версию Oracle Client. Атрибут Connection.versionпозволяет определить, к какой версии Oracle Database обращается соединение. Эти атрибуты затем можно использовать для соответствующей настройки поведения приложения.

Примечание. Oracle ведет список версий базы данных, для которых можно получить разрешение проблем и исправление ошибок. См. График выпуска текущих версий базы данных .
