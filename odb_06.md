...
- Проверьте базу данных на наличие повреждённых данных и исправьте их. Например, если у вас есть таблица MYTABLE с символьным столбцом MYVALUE, в котором, как вы подозреваете, есть повреждённые значения, вы можете определить проблемные данные, используя запрос `select id from mytable where utl_i18n.validate_character_encoding(myvalue) > 0`, который выведет ключи строк с недопустимыми данными.

Если повреждённые данные невозможно изменить, вы можете передать параметры внутреннему методу `decode()`, используемому python-oracledb, чтобы разрешить их выборку и предотвратить сбой всего запроса. Для этого создайте обработчик [outputtypehandler](6.1.5.1) и установите `encoding_errors`. Например, чтобы заменить повреждённые символы в столбцах с символами:
...
Другие варианты поведения кодека можно выбрать `encoding_errors`, см. [раздел Обработчики ошибок](https://docs.python.org/3/library/codecs.html#error-handlers).
...
Не подставляйте значения в запрос с использованием конкатенации строки с SQL-запросом, используйте bind переменные. См. [раздел «Использование переменных привязки»](odb_08.md).

При обработке нескольких значений данных используйте `Cursor.executemany()` или `Connection.direct_path_load()` для повышения производительности. См. [раздел «Пакетный оператор и Операции массового копирования»](odb_09.md).
...
Чтобы внести изменения, необходимо вызвать:
...
Если атрибут `Connection.autocommit` равен `True`, то каждый выполненный оператор автоматически фиксируется без необходимости вызова `Connection.commit()`. Однако чрезмерное использование атрибута приводит к дополнительной нагрузке на базу данных и может нарушить согласованность транзакций.

Более подробно о методе фиксации и отката изменений данных см. [в разделе Управление транзакциями](odb_10.md).
