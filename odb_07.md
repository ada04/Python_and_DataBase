...
Примеры в этой главе демонстрируют одиночные вызовы с использованием `Cursor.callproc()`, `Cursor.callfunc()` или `Cursor.execute()`. Примеры повторных вызовов с использованием `Cursor.executemany()` приведены в разделе [Пакетное выполнение PL/SQL](odb_09.md#9_1_4).

#### Пользовательские процедуры в JavaScript

Вас также может заинтересовать создание пользовательских процедур на JavaScript вместо PL/SQL. См. раздел [«Введение в Oracle Database Multilingual Engine для JavaScript»](https://www.oracle.com/pls/topic/lookup?ctx=dblatest&id=GUID-6AEC4D40-BE51-4DC6-9B8E-22396B5B16DD). Эти процедуры можно вызывать в python-oracledb так же, как и в PL/SQL.

## 7.1. Хранимые процедуры PL/SQL

Для вызова процедур PL/SQL используется метод `Cursor.callproc()`.
...
Для вызова функций PL/SQL используется метод `Cursor.callfunc()`.
...
Oracle Database требует тип, даже для значений NULL. При передаче значения `None` python-oracledb предполагает, что тип — строка. Если это не тот тип, который требуется, вы можете указать его явно. Например, чтобы передать NULL- объект Oracle Spatial SDO_GEOMETRY в хранимую процедуру PL/SQL `procedure myproc(p in sdo_geometry)` Вы можете использовать:
...
Для создания хранимых процедур и пакетов PL/SQL используйте метод `Cursor.execute()` и команду `CREATE`. Например:
...
Вы можете использовать `gettype()`, чтобы получить тип возвращаемого значения функции PL/SQL, и указать его аргументом `callfunc()` в качестве  типа возвращаемого значения. Например:
...
Переменная `r` будет содержать возвращаемое значение функции PL/SQL в виде объекта типа Object. Доступ к её содержимому можно получить с помощью методов, описанных в разделе [«Извлечение объектов и коллекций базы данных Oracle»](odb_06.md#6_1_8) . Вспомогательная функция `dump_object()`, определённая там, служит удобным примером:
...
вы можете вызвать `callproc()` передав переменную `r` из предыдущего примера в методы `callfunc()` или `newobject()` в соответствующей позиции параметра, например:
...
См. [plsql_rowtype.py](https://github.com/oracle/python-oracledb/tree/main/samples/plsql_rowtype.py) для работающего примера.

## 7.7 Использование DBMS_OUTPUT

Стандартный способ вывода данных из PL/SQL — использование пакета [DBMS_OUTPUT](https://www.oracle.com/pls/topic/lookup?ctx=dblatest&id=GUID-C1400094-18D5-4F36-A2C9-D28B0E12FD8C). Обратите внимание, что код PL/SQL, использующий DBMS_OUTPUT, выполняется до завершения, прежде чем какие-либо выходные данные становятся доступны пользователю. Кроме того, другие соединения с базой данных не могут получить доступ к буферу.
...
- Выполните некоторый код PL/SQL, который вызывает `DBMS_OUTPUT.PUT_LINE()` чтобы поместить текст в буфер.

- Вызывайте `DBMS_OUTPUT.GET_LINE()` или `DBMS_OUTPUT.GET_LINES()` до тех пор пока не будет извлечён весь текст из буфера.
...
Неявные результаты позволяют программе на Python использовать курсоры, возвращаемые блоком PL/SQL, без необходимости использования параметров OUT REF CURSOR. Метод `Cursor.getimplicitresults()` может быть использован для этой цели. Требуется Oracle Database 12.1 (или более поздняя версия). Для режима python-oracledb Thick дополнительно требуется Oracle Client 12.1 (или более поздняя версия).
...
Вы также можете задать переменную окружения `ORA_EDITION`, указав имя вашей редакции.
...
Рассмотрим пример, где одна версия функции PL/SQL `Discount` определена в редакции базы данных по умолчанию `ORA$BASE`, а другая версия той же функции — в редакции, созданной пользователем `DEMO`. В редакторе SQL выполните:
...
Создаётся новая редакция с именем «DEMO», и пользователю предоставляется разрешение на использование редакции. Использование `FORCE` необходимо, если у пользователя уже есть один или несколько объектов редактируемого типа, которые также имеют нередактируемые зависимые объекты.
...
Функция `Discount` демонстрационной версии:
