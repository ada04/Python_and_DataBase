...
Oracle Database использует LOB-объекты для хранения больших объёмов данных, таких как текст, изображения, видео и другие мультимедийные форматы. Максимальный размер LOB-объекта (большого объекта) ограничен размером табличного пространства, в котором он хранится.
...
В python-oracledb можно обрабатывать большие объекты длиной до 1 ГБ напрямую, как строки или байты. Это упрощает работу с большими объектами и даёт значительный выигрыш в производительности по сравнению с потоковой обработкой. Однако для этого требуется, чтобы все данные больших объектов находились в памяти Python, что может быть невозможно.

Примеры LOB-объектов смотрите на GitHub .
...
С помощью python-oracledb данные LOB можно вставлять в таблицу путем привязки строк или байтов по мере необходимости:
...
Объекты CLOB и BLOB размером менее 1 ГБ можно запрашивать из базы данных напрямую как строки и байты. Это может быть гораздо быстрее, чем потоковая передача LOB-объектов . Поддержка включается установкой oracledb.defaults.fetch_lobs, или установкой fetch_lobsпараметра при выполнении оператора:
...
Если не установить значение Falseoracledb.defaults.fetch_lobs или эквивалентный параметр выполнения , или не использовать обработчик выходных типов, значения CLOB и BLOB будут извлечены как объекты LOB . Размер объекта LOB можно получить, вызвав , а данные можно прочитать, вызвав :LOB.size()LOB.read()
...
Для потоковой передачи столбца BLOB LOB.read()метод можно вызывать многократно, пока все данные не будут прочитаны, как показано ниже:
...
При вызове процедур PL/SQL с данными, длина которых превышает 32 767 байт, python-oracledb автоматически создаёт временный большой объект (LOB) и передаёт его значение процедуре. Однако, если объём данных, передаваемых процедуре, превышает объём, помещающийся в один блок данных, можно использовать этот метод Connection.createlob()для создания временного большого объекта (LOB). Этот большой объект затем можно читать и записывать так же, как в примерах выше для постоянных больших объектов (LOB).

12.6 Использование BFILE
BFILE — это объекты, хранящиеся в каталоге файловой системы сервера Oracle Database, а не в базе данных. Столбец базы данных типа BFILE хранит ссылку на этот внешний двоичный файл. Каждый столбец BFILE может ссылаться на один внешний файл. BFILE — это типы данных, доступные только для чтения, поэтому вы не можете изменять файл из приложения.

Перед использованием типа данных BFILE необходимо:

Создайте объект DIRECTORY , который является псевдонимом для полного пути к каталогу, содержащему данные BFILE в файловой системе сервера базы данных. Например, вы можете создать объект DIRECTORY следующим образом:
...
Для связи файла и столбца BFILE необходимо использовать функцию BFILENAMEBFILENAME в операторе INSERT. Функция принимает два аргумента: псевдоним каталога и имя файла. Чтобы вставить ссылку BFILE, например:
...
Этот извлеченный LOB-объект можно использовать LOB.fileexists()для проверки того, существует ли файл, на который ссылается LOB-объект типа BFILE.
