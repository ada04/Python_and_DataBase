...
В Python-oracledb есть одноэлементный объект `Defaults` с атрибутами, определяющими поведение драйвера по умолчанию. Доступ к объекту осуществляется через атрибут `defaults` импортированного драйвера.
